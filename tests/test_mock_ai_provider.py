...
def test_mock_provider_rejects_fewer_than_three_chapters():
    provider = MockAIAnalysisProvider()

    with pytest.raises(AIProviderInputError, match="At least 3 chapters are required"):
        provider.analyze_chapters(make_chapters()[:2])
